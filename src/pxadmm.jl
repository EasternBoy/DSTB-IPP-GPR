...
function dstbProxADMM!(robo::Vector{robot}, Pred::Array{Float64}, NB::Vector{Vector{Int64}}, pserSet::Vector{Vector{Int64}}; 
    ...
    p   = Array(Pred) # Initiate
    ξ   = Array{Float64}(undef, Dim, H, M, M)
    λ   = zeros(Dim, H, M, M)
    J   = Vector{Float64}(undef, M)
...
function dstbProjection!(robo::robot, p::Array{Float64, 3}, λ::Array{Float64}, NBi::Vector{Int64}, 
